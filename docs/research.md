...
The next research decision is whether to pre-register a natural-task v2 gate, which would test whether useful trajectories occur without induction, or pivot to simpler post-hoc evidence generation. The artificial diagnostic does not permit reviewer sessions. See the [experiment protocol](experiment-protocol.md) and [roadmap](roadmap.md).
