...
- the hybrid conditional-value prerequisite passed on two controlled non-trivial tasks: both captured one failed path and superseding revision, and each produced one audited runtime-unique relevant claim not verifiable from final-state evidence; the two condition-masked packets are frozen but no reviewer result exists yet; the full preparation cost was 40 model calls, 228,818 reported tokens, about 378 seconds, and about $0.468, including recorded artifact/audit retries and three no-model-cost researcher verification failures;
- no human-comprehension treatment effect has been measured, and the current single-reviewer study cannot estimate one.

The natural runtime-prevalence claim remains stopped. The hybrid prerequisite now establishes that controlled trajectory evidence can survive the artifact and privacy gates; it does not establish reviewer value. The next research step is the user-started masked review of `P02`, followed by `P01`, without opening the condition key between them. See the [experiment protocol](experiment-protocol.md) and [roadmap](roadmap.md).
